...
def _to_sql_datetime(value: datetime | str | None) -> datetime | None:
    """Return a SQL Server-safe, timezone-naive Eastern Time datetime.

    pyodbc binds a ``datetime`` instance as a SQL timestamp and binds ``None``
    as SQL NULL.  This avoids SQL Server attempting to convert ISO timestamp
    strings containing ``T``, ``Z``, or a UTC offset.
    """
    if value is None:
        return None
