...
class Student(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    parent_id: Mapped[int] = mapped_column(ForeignKey("parents.id", ondelete="CASCADE"), index=True, nullable=False)
    display_name: Mapped[str] = mapped_column(String(80), nullable=False)
    grade: Mapped[int] = mapped_column(Integer, default=2, nullable=False)
    preferred_language: Mapped[str] = mapped_column(String(20), default="both", nullable=False)
    beta_code: Mapped[str] = mapped_column(String(20), default=lambda: f"BETA-{secrets.token_hex(3).upper()}", unique=True, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    ...
    feedback: Mapped[list["Feedback"]] = relationship(back_populates="student", cascade="all, delete-orphan")
...
class Feedback(Base):
    ...
    useful_rating: Mapped[str] = mapped_column(String(20), nullable=False)
    ...
    regular_use: Mapped[str] = mapped_column(String(20), nullable=False)
    willingness_to_pay: Mapped[str] = mapped_column(String(30), nullable=False)
    requested_features: Mapped[str] = mapped_column(Text, default="", nullable=False)
    comments: Mapped[str] = mapped_column(Text, default="", nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, index=True)

    student: Mapped[Student | None] = relationship(back_populates="feedback")
